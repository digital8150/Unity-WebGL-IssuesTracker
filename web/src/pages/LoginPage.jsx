import React, { useState, useCallback } from 'react';
import { useLocation, useSearchParams } from 'react-router-dom';
import { useI18n } from '../i18n.jsx';
import BrandLogo from '../components/BrandLogo.jsx';
import DarkModeToggle from '../components/DarkModeToggle.jsx';
import TurnstileWidget from '../components/TurnstileWidget.jsx';
import PageLink from '../components/PageLink.jsx';
import './AuthPage.css';

const API_BASE = import.meta.env.VITE_API_BASE ?? '';
// When no site key is configured, skip the challenge (dev mode).
const SKIP_CHALLENGE = !import.meta.env.VITE_TURNSTILE_SITE_KEY
  || import.meta.env.VITE_TURNSTILE_SITE_KEY === '1x00000000000000000000AA';

const AUTH_RETURN_TARGET_KEY = 'auth.returnTarget';
const UNSAFE_RETURN_PATHS = ['/login', '/auth/callback', '/consent', '/pending'];

function getSafeReturnTarget(value) {
  const candidate = typeof value === 'string'
    ? value
    : value && typeof value === 'object'
      ? `${value.pathname || ''}${value.search || ''}${value.hash || ''}`
      : '';

  if (!candidate || !candidate.startsWith('/') || candidate.startsWith('//') || candidate.includes('\\')) {
    return '/';
  }

  try {
    const url = new URL(candidate, window.location.origin);
    if (url.origin !== window.location.origin) return '/';
    const target = `${url.pathname}${url.search}${url.hash}`;
    const path = url.pathname.replace(/\/$/, '') || '/';
    if (UNSAFE_RETURN_PATHS.some((unsafePath) => path === unsafePath || path.startsWith(`${unsafePath}/`))) {
      return '/';
    }
    return target;
  } catch {
    return '/';
  }
}

function readStoredReturnTarget() {
  try {
    return getSafeReturnTarget(window.sessionStorage.getItem(AUTH_RETURN_TARGET_KEY));
  } catch {
    return '/';
  }
}

function storeReturnTarget(target) {
  try {
    window.sessionStorage.setItem(AUTH_RETURN_TARGET_KEY, target);
  } catch {
    // Private browsing modes may disable sessionStorage; root remains the safe fallback.
  }
}

export default function LoginPage() {
  const { lang, toggleLang, t } = useI18n();
  const [searchParams] = useSearchParams();
  const location = useLocation();
  const oauthError = searchParams.get('error');
  const [cfPassed, setCfPassed] = useState(SKIP_CHALLENGE);

  const requestedReturnTarget = getSafeReturnTarget(location.state?.from);
  const returnTarget = requestedReturnTarget !== '/'
    ? requestedReturnTarget
    : oauthError
      ? readStoredReturnTarget()
      : '/';

  const errorMessage = oauthError
    ? oauthError.startsWith('discord')
      ? t.auth.discordFailed
      : t.auth.githubFailed
    : null;

  const handleToken = useCallback(() => setCfPassed(true),  []);
  const handleExpire = useCallback(() => setCfPassed(false), []);

  function handleOAuthClick(e, url) {
    if (!cfPassed) {
      e.preventDefault();
      return;
    }
    storeReturnTarget(returnTarget);
    window.location.href = url;
  }

  return (
    <div className="auth-page">
      <nav className="auth-topbar">
        <PageLink to="/" className="auth-logo"><BrandLogo /></PageLink>
        <div className="auth-topbar-right">
          <button className="l-lang-toggle auth-lang-toggle" onClick={toggleLang}>
            {lang === 'en' ? '한국어' : 'English'}
          </button>
          <DarkModeToggle />
        </div>
      </nav>

      <div className="auth-card">
        <h1 className="auth-title">{t.auth.loginTitle}</h1>
        <p className="auth-subtitle">{t.auth.loginSub}</p>

        {errorMessage && <div className="auth-error">{errorMessage}</div>}

        {!SKIP_CHALLENGE && (
          <div className="auth-turnstile">
            <TurnstileWidget onToken={handleToken} onExpire={handleExpire} />
          </div>
        )}

        <div className="auth-oauth-stack" style={{ opacity: cfPassed ? 1 : 0.45, pointerEvents: cfPassed ? 'auto' : 'none' }}>
          <a
            className="btn-oauth btn-github"
            href={`${API_BASE}/api/auth/github`}
            onClick={(e) => handleOAuthClick(e, `${API_BASE}/api/auth/github`)}
            aria-disabled={!cfPassed}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61-.546-1.385-1.335-1.755-1.335-1.755-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
            </svg>
            {t.auth.githubLogin}
          </a>

          <a
            className="btn-oauth btn-discord"
            href={`${API_BASE}/api/auth/discord`}
            onClick={(e) => handleOAuthClick(e, `${API_BASE}/api/auth/discord`)}
            aria-disabled={!cfPassed}
          >
            <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
              <path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057c.002.022.015.043.032.054a19.9 19.9 0 0 0 5.993 3.03.077.077 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z" />
            </svg>
            {t.auth.discordLogin}
          </a>
        </div>

        {!cfPassed && !SKIP_CHALLENGE && (
          <p className="auth-turnstile-hint">{t.auth.turnstileHint}</p>
        )}

        <p className="auth-footer-note">{t.auth.oauthNote}</p>
      </div>
    </div>
  );
}
