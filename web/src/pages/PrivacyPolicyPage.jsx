...
export default function PrivacyPolicyPage() {
  ...
  useDocumentMeta({
    title: `${lang === 'en' ? 'Privacy Policy' : '개인정보처리방침'} — BCSDLab. Arcade`,
    description: lang === 'en'
      ? 'Learn how BCSDLab. Arcade collects, uses, stores, and protects personal information.'
      : 'BCSDLab. Arcade 개인정보처리방침입니다.',
    url: `${window.location.origin}${withLocale(date ? `/privacy/${date}` : '/privacy', lang)}`,
    type: 'website',
    robots: isPast ? 'noindex,follow' : undefined,
    alternates: isPast ? null : undefined,
  });
  ...
  return (
    <div className="pp-page">
      ...
      <article className="pp-article">
        <h1>{lang === 'en' ? 'Privacy Policy' : '개인정보처리방침'}</h1>
        <p className="pp-effective">{lang === 'en' ? 'Effective date' : '시행일자'}: {current.effectiveDate}</p>

        {isPast && (
          <p className="pp-history-notice">
            {lang === 'en' ? 'This is a past version of the privacy policy.' : '이 문서는 과거에 시행되었던 버전입니다.'}{' '}
            <PageLink to="/privacy">{lang === 'en' ? 'View the latest privacy policy →' : '최신 개인정보처리방침 보기 →'}</PageLink>
          </p>
        )}
        ...
        {PRIVACY_POLICY_VERSIONS.length > 1 && (
          <>
            <h2>{lang === 'en' ? 'Previous versions' : '이전 버전'}</h2>
